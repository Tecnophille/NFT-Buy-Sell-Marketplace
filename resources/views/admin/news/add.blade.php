                            ...
                            <form method="POST" action="{{route('admin_store_news')}}" enctype="multipart/form-data" class="admin-add-news-page">
                                ...
                                @foreach (allLanguage() as $al)
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="exampleInputEmail1" name="title_{{$al->prefix}}" placeholder="{{__('Enter title')}}">
                                    </div>
                                    <div class="form-group">
                                        <label for="description">{{__('Description')}} ({{$al->name}})</label>
                                        <textarea id="description" class="form-control summernote" name="description_{{$al->prefix}}"></textarea>
                                    </div>
                                    <hr>
                                @endforeach
                                <div class="form-group">
