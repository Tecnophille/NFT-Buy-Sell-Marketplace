                    ...
                    <div class="search-rightside-area">
                        <div class="table-responsive">
                            <table id="serviceList" class="user-dashboard-table table table-striped table-bordered data-table display responsive">
                                <thead>
                                <tr>
                                    <th class="all">{{__('main.ID')}}</th>
                                    <th class="none">{{__('main.Title')}}</th>
                                    <th class="none">{{__('main.Description')}}</th>
                                    <th class="all">{{__('main.Bid Amount(USD)')}}</th>
                                    <th class="all">{{__('main.Highest Bid')}}</th>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header p-0">
                        <h4 class="withdrawal-header modal-title">{{__('main.Resell Product')}}</h4>
                    ...
                    <div class="modal-body p-0 w-ajax-alert">
                        <form action="{{route('resell_service', encrypt($service->service_id))}}" method="POST">
                            ...
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-12 col-lg-12">
                                        <label for="type">{{__('main.Type')}}</label>
                                        <select name="type" class="form-control" id="type" required>
                                            <option value="">{{__('main.---SELECT TYPE---')}}</option>
                                            <option value="{{FIXED_PRICE}}">{{__('main.Fixed Price')}}</option>
                                            <option value="{{BID_PRICE}}">{{__('main.Bid')}}</option>
                                        </select>
                                    </div>
                            ...
                            <div class="form-group d-none" id="price_dollar_d">
                                <div class="row">
                                    <div class="col-12 col-lg-12">
                                        <label for="type">{{__('main.Price')}}</label>
                                        <input class="form-control" placeholder="{{__('main.New Price')}}" name="price_dollar" value="0" id="price_dollar" step="0.01" type="number" min="0" max="9999">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group d-none" id="min_bid_price_d">
                                <div class="row">
                                    <div class="col-12 col-lg-12">
                                        <label for="type">{{__('main.Minimum Bid Amount')}}</label>
                                        <input class="form-control" placeholder="{{__('main.Minimum Bid')}}" name="min_bid_price" id="min_bid_price" value="0.01" step="0.01" type="number" min="0" max="9999">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group d-none" id="max_bid_price_d">
                                <div class="row">
                                    <div class="col-12 col-lg-12">
                                        <label for="type">{{__('main.Maximum Bid Amount')}}</label>
                                        <input class="form-control" placeholder="{{__('main.Maximum Bid')}}" name="max_bid_price" id="max_bid_price" value="999999" step="0.01" type="number" min="0" max="999999">
                                    </div>
                                </div>
                            </div>
                            <div class="sign-up-button-part">
                                <button type="submit"  class="theme-button1 d-none" id="f-submit">{{__('main.Submit')}}</button>
                            </div>
                        </form>
                    </div>
                </div>
