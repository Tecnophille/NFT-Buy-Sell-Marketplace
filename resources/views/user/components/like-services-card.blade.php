    ...
    <div class="col-12 col-sm-6 col-md-4 col-lg-4">
        <div class="explore-item user-profile-item">
            <div class="artist-img position-relative">
                <img src="{{cdnAsset(IMG_SERVICE_PATH,$cu->service->thumbnail)}}" alt="explore-img" class="img-fluid">
                <div class="artist-overlay position-absolute">
                    ...
                    @if ($cu->service->type == 1)
                        <a href="{{route('product_view', encrypt($cu->service->id))}}" class="place-a-bid-btn">{{__('main.Purchase Now')}}</a>
                    @elseif($cu->service->type == 2)
                        <a href="{{route('product_view', encrypt($cu->service->id))}}" class="place-a-bid-btn">{{__('main.Place a Bid')}}</a>
                    @endif
                </div>
            </div>
            <div class="explore-content">
                ...
                <div class="explore-small-box explore-author-wrap d-flex align-items-center justify-content-between">
                    <div class="explore-author d-flex align-items-center">
                        <p>{{__('main.Uploaded')}} <span>{{\Carbon\Carbon::parse($cu->service->created_at)->format('jS F')}}</span></p>
                    </div>
                    <div class="like-box">
                ...
                <div class="explore-small-box d-flex align-items-center justify-content-between">
                    <p class="on-sell">{{__('main.Status :')}} <span>{{service_status($cu->service->id)}}</span></p>
